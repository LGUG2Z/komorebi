...
pub struct StorageConfig {
    ...
    /// Select when the current percentage is over this value [[1.0-100.0]]
    pub auto_select_over: Option<f32>,
    /// Hide when the current percentage is under this value [[1.0-100.0]]
    pub auto_hide_under: Option<f32>,
}
...
impl From<StorageConfig> for Storage {
    fn from(value: StorageConfig) -> Self {
        Self {
            ...
            auto_select_over: value.auto_select_over.map(|o| o.clamp(1.0, 100.0)),
            auto_hide_under: value.auto_hide_under.map(|o| o.clamp(1.0, 100.0)),
...
pub struct Storage {
    ...
    auto_select_over: Option<f32>,
    auto_hide_under: Option<f32>,
...
impl Storage {
    fn output(&mut self) -> Vec<StorageDisk> {
        ...
        for disk in &self.disks {
            ...
            let hide = self
                .auto_hide_under
                .is_some_and(|u| (percentage as f32) < u);

            if !hide {
                let selected = self.auto_select_over.is_some_and(|o| percentage as f32 > o);
